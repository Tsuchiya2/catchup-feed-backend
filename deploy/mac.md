...
**新しい launchd・cron・常駐物はない**。9章で登録済みの radio バッチ(04:30)に
学習ループが同居し、同じランで毎朝、私的エピソード(news + 復習クイズ +
book_review)を生成して私的フィードに載せる。公開エピソードの構成・尺・通知と
友人の体験は一切変わらない(§12-1)。Mac が閉じている日は公開版と同様に欠番
(従来どおりの縮退)。
...
### 13.2 私的フィードへの購読切り替え【ユーザー作業】(§3)

クイズ・書籍入りのエピソードは私的フィードにしか載らない。本人のポッドキャスト
アプリの購読を、公開フィード(トークン URL)から私的フィードへ切り替える:

1. スマホの Tailscale アプリが接続中であることを確認(Phase 2 の Open WebUI
   PWA(12.4)で tailnet 参加済み。追加のセットアップは不要)
2. ポッドキャストアプリに購読を追加:
   ```
   http://<pi の MagicDNS 名>:8081/private/feed.xml
   ```
   pi.md 7章の私的フィード動作確認と同じ URL(MagicDNS 名の代わりに Pi の
   tailnet IP でもよい)。トークンは不要 — tailnet 参加そのものが認証(C-5)で、
   compose がこのリスナーを tailnet アドレスにしかバインドしないため、
...
切り替え後の見え方: 毎朝、同タイトルのエピソードが1件だけ並ぶ(公開/私的ペアは
私的版に畳まれ、私的版が生成されなかった日は公開版がそのまま載る)。Tailscale が
切断されているとフィード更新は一時的に失敗するが、再接続後の次回リフレッシュで戻る。

### 13.3 書籍レビュー(book_review)

- 対象書籍の指定はダッシュボードの `/learning/books` から **activate** する(D-20)。
  同時に進行するのは最大1冊。activate 中の書籍がない日は book_review なしの構成
  (news+quiz)になるだけで異常ではない
- モデルは 11章で pull 済みの **gemma4:12b** をそのまま使う
  (`BOOK_REVIEW_OLLAMA_MODEL` で変更可)。書籍は私的データのため生成は
  **ローカル Ollama 限定**で、クラウド API には乗らない(C-12)
- Ollama 不調・生成失敗の日は book_review だけスキップされ、翌日カーソル位置から
  再開する(縮退)。復旧操作は不要

### 13.4 採点フロー

- 私的版ショーノートの採点ページリンク(`RADIO_LEARNING_URL`、既定
  `https://pulse.catchup-feed.com/learning`)からダッシュボードを開き、出題項目を
  ○(覚えていた)/△(あやふや)/×(忘れていた)で採点する
- 採点は任意。未採点の出題は 48h(`QUIZ_AUTO_RESOLVE_AFTER`)経過で「覚えていた」
  相当として自動前進する(D-17)。採点しない日があってもキューは自然に流れる
