...
窓内でも **04:14 を過ぎた発火は worker を起動せず、radio までのブリッジだけ張って
正常終了**する(worker の deadline は「今より後の次の 04:15」に解決されるため、
04:15 以降に起動すると翌朝まで走ってしまう — その帯を worker なしで塞ぐ)。
手動実行は引数を渡すか端末から実行すればどちらのガードも対象外 — この章の手順どおり
