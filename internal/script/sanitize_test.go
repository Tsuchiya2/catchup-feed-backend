...
func TestSanitizeScript_ClosingBracketBeforeParticle(t *testing.T) {
	tests := []struct {
		...
		// --- N-a: **許容する誤爆**。語頭が助詞と同じかなで始まる語の前でも
		// 読点が消える。「ところが」「もっとも」は本来 pause が欲しい接続詞
		// なので、ここだけ見れば N-4 前のほうが自然である。
		//
		// それでもこの表は**増やさない**。助詞の「と」と接続詞「ところが」の
		// 語頭「と」は、形態素解析なしには区別できない(設計原則1: 解析器を
		// 持ち込まない)。閉じ括弧の後ろに助詞が来る頻度のほうが高く期待値は
		// プラスで、外したときの代償も読点1つが消えることに留まる — N-3 の
		// 閾値表と同じ流儀で、誤爆を承知のうえで狭い実装を選んでいる。
		// **「助詞表を増やせば良くなる」わけではない**ことをここで固定する。
		{"注記（付録）ところが状況が変わりました。", "注記、付録ところが状況が変わりました。"},
		{"補足（略）もっとも重要です。", "補足、略もっとも重要です。"},
	}
	for _, tt := range tests {
