...
// mockDiscordNotifier is a test implementation of the Notifier interface
// used to test DiscordChannel behavior without making real HTTP requests.
...
func (m *mockDiscordNotifier) NotifyArticle(ctx context.Context, article *entity.Article, source *entity.Source) error {
	m.notifyCalled++
	m.capturedCtx = ctx
...
// newTestDiscordChannel creates a DiscordChannel with a mock notifier for testing.
func newTestDiscordChannel(enabled bool, mockNotifier *mockDiscordNotifier) *DiscordChannel {
...
// TestDiscordChannel_Name verifies the Name method returns "discord".
...
// TestDiscordChannel_IsEnabled verifies the IsEnabled method returns the config value.
...
// TestDiscordChannel_Send_DelegatesToNotifier verifies that Send delegates to NotifyArticle.
func TestDiscordChannel_Send_DelegatesToNotifier(t *testing.T) {
	// Arrange
	...
	ch := newTestDiscordChannel(true, mockNotifier)
...
// TestDiscordChannel_Send_PropagatesErrors verifies that Send propagates errors from the notifier.
func TestDiscordChannel_Send_PropagatesErrors(t *testing.T) {
	tests := []struct {
		name          string
		enabled       bool
		...
		},
		{
			...
			notifierError: errors.New("Discord rate limit exceeded (retry after 5s)"),
			wantErr:       errors.New("Discord rate limit exceeded (retry after 5s)"),
			wantCalled:    1,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ch := newTestDiscordChannel(tt.enabled, mockNotifier)
...
// TestDiscordChannel_Send_RespectsContext verifies that Send respects context cancellation.
func TestDiscordChannel_Send_RespectsContext(t *testing.T) {
	// Arrange
...
// TestDiscordChannel_Send_WithTimeout verifies timeout behavior.
func TestDiscordChannel_Send_WithTimeout(t *testing.T) {
	// Arrange
...
// TestDiscordChannel_NewDiscordChannel_WithDisabledConfig verifies NoOpNotifier is used when disabled.
func TestDiscordChannel_NewDiscordChannel_WithDisabledConfig(t *testing.T) {
	// Arrange
	config := notifier.DiscordConfig{
		Enabled:    false,
		WebhookURL: "",
...
// TestDiscordChannel_NewDiscordChannel_WithEnabledConfig verifies DiscordNotifier is used when enabled.
func TestDiscordChannel_NewDiscordChannel_WithEnabledConfig(t *testing.T) {
	// Arrange
	config := notifier.DiscordConfig{
		Enabled:    true,
	...
	if ch.Name() != "discord" {
		t.Errorf("Name() = %v, want discord", ch.Name())
	}

	// Verify notifier is not nil
	if ch.notifier == nil {
		t.Error("notifier is nil, want DiscordNotifier instance")
	}
}
