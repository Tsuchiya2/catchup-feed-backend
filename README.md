...
> **Go 1.25.4 / クリーンアーキテクチャ / PostgreSQL 18 / Claude Sonnet 4.5**
...
**関連ドキュメント:**
- **Claude Code向けルール:** [.claude/CLAUDE.md](.claude/CLAUDE.md) - EDAF v1.0 エージェントシステム、4段階ゲートシステム
...
> 💡 **推奨: Docker開発環境を使用**
> ローカルにGoをインストール不要！詳細は [docs/DEVELOPMENT.md](docs/DEVELOPMENT.md) を参照
...
すべてのコマンドは [Makefile](Makefile) または [docs/DEVELOPMENT.md](docs/DEVELOPMENT.md) を参照
...
詳細は [`docs/deployment/notification-system-rollout.md`](docs/deployment/notification-system-rollout.md) を参照してください。
...
詳細なデプロイメント手順は [`docs/deployment/notification-system-rollout.md`](docs/deployment/notification-system-rollout.md) を参照してください。
...
詳細: [docs/security/authentication-requirements.md](docs/security/authentication-requirements.md)
...
### EDAF v1.0 - 4段階ゲートシステム

catchup-feedプロジェクトでは、**EDAF v1.0**（Enterprise Development Automation Framework）を採用しています。

**キーワード：「エージェントフロー」または「EDAF」**

**使用例:**
- 「エージェントフローに沿って、記事タグ機能を追加してください」
- 「EDAFに沿って、要約エンジンをリファクタリングしてください」

**適用場面:**
- ✅ 複雑なタスク（新機能追加、リファクタリング）
- ✅ クリティカルなタスク（認証、セキュリティ）
- ✅ 高品質保証が必要なタスク
- ✅ 完全自動化を希望する場合

### EDAF 4-Phase Gate System

**Phase 1: Design Gate（設計）**
- Designer エージェント → 設計書作成（`docs/designs/{feature-slug}.md`）
- 7つのDesign Evaluators → 並列評価（一貫性、拡張性、保守性等）
- 全評価 ≥ 7.0/10.0 で次フェーズへ

**Phase 2: Planning Gate（計画）**
- Planner エージェント → タスク計画作成（`docs/plans/{feature-slug}-tasks.md`）
- 7つのPlanner Evaluators → 並列評価（明確性、粒度、依存関係等）
- 全評価 ≥ 7.0/10.0 で次フェーズへ

**Phase 2.5: Implementation（実装）**
- Worker エージェント → コード実装
...
**Phase 3: Code Review Gate（レビュー）** ← **絶対にスキップ禁止**
- 7つのCode Evaluators → 並列評価（品質、セキュリティ、パフォーマンス等）
- フロントエンド変更時は MCP chrome-devtools で UI/UX 検証
- 全評価 ≥ 7.0/10.0 で次フェーズへ

**Phase 4: Deployment Gate（デプロイ）** - オプション
- 5つのDeployment Evaluators → 並列評価（準備状況、セキュリティ、監視等）
- 全評価 ≥ 7.0/10.0 でデプロイ承認
...
│   │   └── notify/           # 通知ユースケース（NEW）
│   ├── handler/
│   │   └── http/             # HTTPハンドラ（article, source, auth）
│   │       ├── article/      # 記事ハンドラ
│   │       ├── source/       # ソースハンドラ
...
│   └── pkg/                  # 共通パッケージ
├── docs/                     # プロジェクトドキュメント
│   ├── designs/              # 設計書（Designer エージェントが自動作成）
│   ├── plans/                # タスク計画（Planner エージェントが自動作成）
│   ├── reviews/              # レビューレポート（Code Evaluators が自動作成）
│   ├── screenshots/          # UI/UX スクリーンショット（chrome-devtools）
│   ├── reports/              # 各種レポート
│   └── deployment/           # デプロイメントドキュメント（NEW）
├── .claude/                  # EDAF v1.0 設定
│   ├── CLAUDE.md             # EDAF v1.0 システムガイド
│   ├── agents/               # エージェント定義（designer, planner, workers）
│   ├── evaluators/           # 評価エージェント定義（26個）
│   ├── commands/             # カスタムコマンド（/setup）
│   ├── scripts/              # 通知スクリプト
│   ├── sounds/               # 通知サウンド
│   ├── settings.json         # Claude Code設定、MCP権限
...
| ドキュメント | 用途 |
|------------|------|
| **[.claude/CLAUDE.md](.claude/CLAUDE.md)** | EDAF v1.0 エージェントシステム、4段階ゲートシステム詳細 |
...
**ドキュメント最終更新:** 2025-11-11
**EDAF v1.0 移行完了:** 2025-11-11
