...
- **ルート直下の `swagdep.go`(ビルドタグ `swagdep`、どこでも設定しない)を消さない**。`github.com/swaggo/swag` を direct 依存として固定するためだけのファイル。swag を実際に import するのは生成物 `docs/docs.go` だが gitignore 済みのため、これが無いと go.mod の tidy 結果がツリーの状態に依存し、生成物の無いツリーで tidy する dependabot の gomod PR が `// indirect` への降格を持ち込む(#122・#124・#130 で3回発生)。go.mod の `tool` ディレクティブでは防げない — `// indirect` は「main module のパッケージが import していない」ことを表すマーカーで、tool ディレクティブはその判定に参加しない(`go get -tool` 単体でも `// indirect` が付く)。**疑ったら使い捨てのツリーで30秒で確認できる**: `rm docs/docs.go swagdep.go && go mod tidy` すると、tool 行が go.mod に残ったままでも swag が `// indirect` に落ちる(`swagdep.go` だけ戻して再度 tidy すれば direct に戻る)。確認後は `git restore .` と `make swagger-host` で戻す。go.mod 側の3者は役割が別: **バージョンを選ぶのは `require github.com/swaggo/swag v1.16.6`**、`tool github.com/swaggo/swag/cmd/swag` は `go tool swag` で呼ぶツールのパッケージパスを名指すだけ、`swagdep.go` は swag を direct 依存として require に居続けさせるだけ(後の2つはバージョンを持たない)。バージョンを上げる操作は従来どおり `go get github.com/swaggo/swag@vX.Y.Z` の1回で全経路に反映される
